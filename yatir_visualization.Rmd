...
```{python}
# finally get a quadmesh with two dimensions that can vary!  needs kernel wrfhv_github because
# that kernel has latest hvplot from github (not from conda), which includes the fix for the 
# bug that was causing problems (https://github.com/holoviz/holoviews/pull/4188).
# TODO: create one set of widgets that all plots respond to.
def quadmesh_multidim(ds, varname, WRFrun_arg, widgets=None):
    """temporary wrapper function to display a quadmesh with more than one slider
    ds: concatenated xarray dataset from above
    """
    vertical_dim = get_vdim(ds, varname)
    qm = ds[varname].sel(WRFrun=WRFrun_arg).hvplot.quadmesh(
        x=dims['lon'], 
        y=dims['lat'], 
        groupby=['hour', vertical_dim])
    return(qm)
```

```{python}
def get_quadmesh(ds, varname, WRFrun_arg, hour, z_level):
    """
    """
    vertical_dim = get_vdim(ds, varname)
    qm = ds[varname].sel(WRFrun=WRFrun_arg, 
                         hour=hour, 
                         bottom_top_stag=z_level).hvplot.quadmesh()
    return(qm)
```

```{python}
qm1 = get_quadmesh(ds_diff, 'W', 'control', 9, 5)
qm1
```

```{python active="", eval=FALSE}
hour_select = pn.widgets.IntSlider(start=0, end=24, value=9)
z_select = pn.widgets.IntSlider(start=0, end=25, value=2)
WRFrun_arg = 'control'
@pn.depends(hour_select.param.value, z_select.param.value, WRFrun_arg)
def get_quadmesh_slider(hour_select, z_select, WRFrun_arg):
    """
    """
    qm = ds_diff['W'].sel(WRFrun=WRFrun_arg,
                     hour=hour_select,                         
                     bottom_top_stag=z_select).hvplot.quadmesh()
    return(qm)
```

```{python active="", eval=FALSE}
pn.Row(pn.Column(hour_select, z_select, get_quadmesh_slider(hour_select, 
                                                            z_select,
                                                           'control')),
       get_quadmesh_slider(hour_select, 
                           z_select,
                          'yatir'))
```

```{python}
class YatirCompareQuadmesh(param.Parameterized):
    hour_select = param.Integer(9, bounds=(1, 24))
    z_select = param.Integer(3, bounds=(0, 30))
    
    @param.depends('hour_select', 'z_select')
    def get_quadmesh(self, WRFrun_arg):
        """
        """
        qm = ds_diff['W'].sel(WRFrun=WRFrun_arg,
                         hour=self.hour_select,                         
                         bottom_top_stag=self.z_select).hvplot.quadmesh()
        return(qm)
    
```

```{python}
qm_W = YatirCompareQuadmesh()
pn.Column(pn.Row(qm_W.get_quadmesh('control'), qm_W.get_quadmesh('yatir')),
          qm_W.param)
```
...
pn.extension()
qm1 = get_quadmesh(ds_diff, 'W', 'control', 9, 5)
qm2 = get_quadmesh(ds_diff, 'W', 'yatir', 9, 5)
pn.Row(qm1, qm2)
```

```{python}
quadmesh_multidim(ds_diff, 'W')
```

```{python}
quadmesh_multidim(ds_diff, 'theta')
```

```{python}
quadmesh_multidim(ds_diff, 'height_agl')
```
