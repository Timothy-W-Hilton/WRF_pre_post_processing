...
def quadmesh_multidim(ds, varname):
    ...
    qm = [ds_diff[varname].sel(WRFrun=WRFrun).hvplot.quadmesh(x=dims['lon'], 
                                                              y=dims['lat'], 
                                                              groupby=['hour', vertical_dim])
          for WRFrun in ds_diff['WRFrun']]
    return(qm)
```

```{python}
import panel as pn
foo = quadmesh_multidim(ds_diff, 'W')
p = pn.Row(*foo)
p
```
