...
d = ds['control', 'W'].data
d2 = d.assign_coords({'bottom_top_stag': np.arange(d.dims['bottom_top_stag'])})
d2.hvplot.quadmesh(x=dims['lon'], y=dims['lat'], groupby=['hour', 'bottom_top_stag'])
```

```{python}
make_var_layout(ds, 'W')
```

```{python}
gv.extension('matplotlib')
make_var_layout(ds, 'T')
```

```{python}
make_var_layout(ds, 'theta')
```

```{python}
make_var_layout(ds, 'height_agl')
```
