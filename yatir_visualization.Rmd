...
ctlday_TP, ytrday_TP, ctl_minus_ytr_TP = gt.merge_yatir_fluxes_landuse(
    fname_ctl='ctl_run_d03_diag_TP.nc',
    fname_yatir='yatir_run_d03_diag_TP.nc')
```

```{python}
ctlday = xr.merge((ctlday, ctlday_TP))
ytrday = xr.merge((ytrday, ytrday_TP))
ctl_minus_ytr = xr.merge((ctl_minus_ytr, ctl_minus_ytr_TP))
```
...
ds = {(WRFrun, var): gv.Dataset(ds_diff.sel(WRFrun=WRFrun)[var], 
       ...
       for var in ['W', 'T', 'SWDOWN', 'HFX', 'LH', 'GPP', 'FVEG', 'FSA', 'FIRA', 'RSSUN', 'RSSHA']}
