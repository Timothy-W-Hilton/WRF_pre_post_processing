...
cv_obs = hv_ytr.to.curve('time', dims['HFX'], [], label='Yatir observed')
df_ytr[['HFX', 'time', 'hour']]
obs_mean = df_ytr[['HFX', 'LH', 'hour']].groupby('hour').mean()
```
...
```{python}
ds_obs_mean = xr.DataArray(data=obs_mean['HFX'].values[np.newaxis, np.newaxis, :], 
                           coords={'hour': (['hour'], obs_mean.reset_index()['hour']),
                                   'area': (['area'], ['yatir']),
                                   'WRFrun': (['WRFrun'], ['Yatir obs'])},
                           dims=('area', 'WRFrun', 'hour'))
means_with_obs = xr.concat((means['HFX'], ds_obs_mean), dim='WRFrun')
```

```{python}
hv.Dataset(means_with_obs).to(hv.Curve, kdims='hour', vdims=dims['HFX']).overlay('WRFrun').layout('area')
```
