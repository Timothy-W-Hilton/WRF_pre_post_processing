...
means = xr.concat([ds_diff.where(mask).mean(dim=['south_north', 'west_east'])
                   for mask in [is_yatir, is_desert]],
                  dim=pd.Index(['yatir', 'desert',], name='area'))
plots = hv.Dataset(means['HFX']).to(hv.Curve, kdims='hour').opts(xlim=(0, 24))
plots.overlay('WRFrun').layout('area')
```
...
means_with_obs = gt.combine_yatir_obs_WRF(obs_mean, means, 'HFX')
```

```{python}
foo = hv.Dataset(means_with_obs.rename('HFX')).to(hv.Curve, kdims='hour', vdims=dims['HFX']).opts(width=400).overlay('WRFrun').opts(legend_position='right').layout('area')
foo
```

```{python}
gt.daily_cycle_mean_overlay_layout(means_with_obs, 'HFX')
```

```{python}
means_with_obs = gt.combine_yatir_obs_WRF(obs_mean, means, 'LH')
hv.Dataset(means_with_obs).to(hv.Curve, kdims='hour', vdims=dims['LH']).overlay('WRFrun').layout('area')
```

```{python}
means_with_obs = gt.combine_yatir_obs_WRF(obs_mean, means, 'SWDOWN')
hv.Dataset(means_with_obs).to(hv.Curve, kdims='hour', vdims=dims['SWDOWN']).overlay('WRFrun').layout('area')
```

```{python}
means_with_obs = gt.combine_yatir_obs_WRF(obs_mean, means, 'FIRA')
hv.Dataset(means_with_obs).to(hv.Curve, kdims='hour', vdims=dims['FIRA']).overlay('WRFrun').layout('area')
```
