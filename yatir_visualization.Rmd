...
ctlday = gt.WRF_daily_daylight_avg(os.path.join(cscratch_path, 'fluxes_ctl_run_d03.nc'))
ytrday = gt.WRF_daily_daylight_avg(os.path.join(cscratch_path, 'fluxes_yatir_run_d03.nc'))
```
...
Show vegetation fraction output by WRF for the control run and Yatir parameterization run.  WRF is setting the vegetation fraction for Yatir Forest to 0.0.

```{python}
hv.Layout([gv.Dataset(ds.sel(hour=10), 
                      kdims=[dims['lon'], dims['lat']]).to(gv.QuadMesh, 
                                          kdims=[dims['lon'], dims['lat']], 
                                          vdims=dims['FVEG']).opts(cmap=cmaps['FVEG'], 
                                                                   colorbar=True,
                                                                   title=label)
           for label, ds in dict(zip(['control', 'Yatir'], [ctlday, ytrday])).items()]).opts(title=dims['FVEG'].label)
```
