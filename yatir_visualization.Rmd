...
```{python}
hour_select = pn.widgets.IntSlider(start=0, end=10)
@pn.depends(hour_select)
def get_quadmesh_slider(hour_select):
    """
    """
    qm = ds_diff['W'].sel(WRFrun='control',
                     hour=hour_select, 
                     bottom_top_stag=3).hvplot.quadmesh()
    return(qm)
```

```{python}
pn.Row(pn.Column(hour_select, get_quadmesh_slider))
```
